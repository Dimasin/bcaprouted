# Проект микросервиса Bcaprouted.
...
По сути сервис лишь управляет включением и отключением модема, основная работа выполняется компонентами OC: получение DHCP и настройка маршрутизации. Поэтому перед тем как браться настраивать этот сервис нужно убедится в правильной работе сетевых компонетов ОС.
